...
func TestOverloadConfigDirWithEnv(t *testing.T) {
	...
	err := os.MkdirAll(os.TempDir()+"/conf/", 0750)
	if err != nil {
		t.Fatal(err)
	}
	err = os.MkdirAll(os.TempDir()+"/confOverloaded1/", 0750)
	if err != nil {
		t.Fatal(err)
	}

	err = os.MkdirAll(os.TempDir()+"/confOverloaded2/", 0750)
	if err != nil {
		t.Fatal(err)
	}
...
nested:
	...
	if err == nil {
		...
		if !ok {
			t.Fatal("Validation with Tags Returned Wrong Error Plugin.")
		}
		_ = errx.Unwrap()
	}
}
...
nested:
	...
	if Errs == nil {
		...
		for _, err := range Errs {
			...
			if !ok {
				t.Fatal("Validation using validator interface Returned Wrong Error Plugin.")
			}
			_ = errx.Unwrap()
		}
	}
