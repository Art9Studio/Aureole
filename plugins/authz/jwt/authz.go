...
var (
	...
	swaggerJson []byte
	//go:embed docs/responses.json
	responsesJson []byte
)

func (j *authz) Init(api core.PluginAPI) (err error) {
	j.pluginAPI = api
...
func assembleSwagger(j *authz) error {
...
func assembleResponses(j *authz) error {
...
func (j *authz) GetMetaData() plugins.Meta {
	return plugins.Meta{
		Type: adapterName,
		ID:   pluginID,
	}
...
func (j *authz) GetHandlersSpec() (*spec.Paths, spec.Definitions) {
	return j.swagger.Paths, j.swagger.Definitions
}

func (j *authz) GetResponseData() (*spec.Responses, spec.Definitions) {
	return j.responses.Responses, j.responses.Definitions
}

func (j *authz) GetNativeQueries() map[string]string {
	return j.nativeQueries
}

func (j *authz) Authorize(c *fiber.Ctx, payload *plugins.Payload) error {
