...
func (m *manager) Register(c *plugins.Credential, i *plugins.Identity, authnProvider string) (*plugins.Identity, error) {
	...
	if exists {
		...
		if authnProvider == "password_based" {
			password, ok := i.Additional[plugins.Password]
			if ok {
				delete(i.Additional, plugins.Password)
				return registerPWBasedIdentity(conn, i, password.(string))
			}
			return nil, errors.New("cannot find password")
		}
...
func (m *manager) GetData(c *plugins.Credential, authnProvider, name string) (interface{}, error) {
	...
	if exists {
		var data interface{}
		if authnProvider == "password_based" && name == plugins.Password {
			sql := fmt.Sprintf("select password from passwords where user_id=(select id from users where %s=$1)",
				sanitize(c.Name))
...
func registerPWBasedIdentity(conn *pgxpool.Conn, newIdent *plugins.Identity, password string) (*plugins.Identity, error) {
	...
	savePasswdSql := "insert into passwords(user_id, password) values($1, $2)"
	_, err = tx.Exec(context.Background(), savePasswdSql, ident.ID, password)
	if err != nil {
		return nil, err
	}
...
func save2FAData(conn *pgxpool.Conn, cred *plugins.Credential, data *plugins.MFAData) error {
	...
	sql := fmt.Sprintf(`insert into mfa(user_id, plugin_id, provider_name, payload) 
		                      (select id, $1, $2, $3::json from users where %s=$4);`,
		sanitize(cred.Name))
