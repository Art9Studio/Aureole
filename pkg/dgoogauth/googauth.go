...
type OTPConfig struct {
	...
	ScratchCodes  []string // an array of 8-digit codes that can be used to log in
	UTC           bool     // use UTC for the timestamp instead of local time
}

func (c *OTPConfig) checkScratchCodes(code string) bool {
...
func (c *OTPConfig) Authenticate(password string) (bool, error) {
	...
	if scratch {
		return c.checkScratchCodes(password), nil
	}
