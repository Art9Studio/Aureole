...
- Тип: **object**. Обязательны: `api_version`, `apps`. Наличие дополнительных свойств: **Недопустимо**.
  ...
  - `apps`: Тип: **array**. Перечисление приложений. Используется для указания конфигураций всех приложений, которые использует Aureole. Минимальное кол-во элементов: **1**.
    - **Элементы**: Тип: **object**. Конфигурация одного приложения. Используется для конфигурирования одного приложения. Обязательны: `name`, `authN`. Наличие дополнительных свойств: **Недопустимо**.
      ...
      - `id_manager`: Тип: **string**. Дополнительно: Ссылка на экземпляр плагина Identity менеджера. Ссылка на плагин identity менеджера. Имя плагина identity, который будет использовать это приложение.
  ...
  - `id_managers`: Тип: **array**. Перечисление конфигураций менеджеров. Используется для указания конфигураций всех менеджеров, которые используются в проекте.
    - **Элементы**: Тип: **object**. Identity менеджер Описывает все свойства данного identity менеджера. Плагин берет на себя роль взаимодействия с БД и предоставляет ряд методов для получения и сохранения данных пользователя. Обязательны: `name`. Наличие дополнительных свойств: **Недопустимо**.
      - `name`: Тип: **string**. Дополнительно: Имя экземпляра плагина. Имя менеджера. Имя используется для того, чтобы в дальнейшем ссылаться на данный менеджер. Минимальная длина: **1**.
      ...
      - Тип: **object**. Конфигурация jwt webhook identity менеджера. Служит для задания свойств менеджера. Обязательны: `type`, `config`. Наличие дополнительных свойств: **Недопустимо**.
        ...
        - `config`: Тип: **object**. Конфигурация identity менеджера. Описывает все свойства данного менеджера. Обязательны: `address`. Наличие дополнительных свойств: **Недопустимо**.
          ...
          - `retry_interval`: Тип: **number**. Интервал между запросами. Время, которое будет ждать плагин, перед тем, как совершить очередную попытку сделать запрос. Минимальное значение: **0.1**.
          - `timeout`: Тип: **number**. Время ожидания ответа от сервера. Максимальное время ожидание ответа, по истечению которого, плагин перестанет ждать ответа. Минимальное значение: **0.1**.
